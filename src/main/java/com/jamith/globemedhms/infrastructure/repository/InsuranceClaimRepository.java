...
public class InsuranceClaimRepository {
    ...
    public InsuranceClaim findByBillingId(int billingId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<InsuranceClaim> query = session.createQuery(
                    "FROM InsuranceClaim WHERE billing.id = :billingId", InsuranceClaim.class);
            query.setParameter("billingId", billingId);
            InsuranceClaim claim = query.uniqueResult();
            if (claim == null) {
                logger.warn("No insurance claim found for billing ID: {}", billingId);
            } else {
                logger.info("Found insurance claim for billing ID: {}", billingId);
            }
            return claim;
        } catch (Exception e) {
            logger.error("Error finding insurance claim by billing ID: {}", billingId, e);
            return null;
        }
    }
}